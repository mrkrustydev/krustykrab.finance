...
// SousChef is the chef of new tokens.
contract SousChef {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;   // How many PATTY tokens the user has provided.
        uint256 rewardDebt;  // Reward debt. See explanation below.
        uint256 rewardPending;
        //
        // We do some fancy math here. Basically, any point in time, the amount of PATTYs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accRewardPerShare) - user.rewardDebt + user.rewardPending
        //
        // Whenever a user deposits or withdraws PATTY tokens to a pool. Here's what happens:
    ...
    // Info of each user that stakes PATTY tokens.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        uint256 stakedSupply = patty.balanceOf(address(this));
    ...
    function updatePool() public {
        ...
        uint256 pattySupply = patty.balanceOf(address(this));
        if (pattySupply == 0) {
        ...
        poolInfo.accRewardPerShare = poolInfo.accRewardPerShare.add(tokenReward.mul(1e12).div(pattySupply));
        poolInfo.lastRewardBlock = block.number;
    }


    // Deposit patty tokens to SousChef for Reward allocation.
    function deposit(uint256 _amount) public {
        require (_amount > 0, 'amount 0');
        UserInfo storage user = userInfo[msg.sender];
        updatePool();
        patty.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    // Withdraw patty tokens from SousChef.
    function withdraw(uint256 _amount) public {
        ...
        updatePool();
        patty.safeTransfer(address(msg.sender), _amount);
    ...
    function emergencyWithdraw() public {
        UserInfo storage user = userInfo[msg.sender];
        patty.safeTransfer(address(msg.sender), user.amount);
